...
fn TinyVec_splice() {
    ...
    assert_eq!(
        Vec::from_iter(tv.clone().splice(0..=2, None)),
        vec![1, 2, 3]
    );
...
fn TinyVec_from_slice_impl() {
    ...
    let tinyvec: TinyVec<[u8; 10]> =
        TinyVec::Inline(ArrayVec::from_array_len([0, 1, 2, 3, 4, 0, 0, 0, 0, 0], 5));
    assert_eq!(TinyVec::from(&smaller_slice[..]), tinyvec);

    let same_size: [u8; 4] = [0, 1, 2, 3];
    let tinyvec: TinyVec<[u8; 4]> = TinyVec::Inline(ArrayVec::from_array_len(same_size, 4));
